...
struct xlb_base_func_ptr {
    ...
    using onregistry_t = int (*)(lua_State* L, xlb_base_func_ptr*);
    ...
    xlb_base_func_ptr(onregistry_t callback = nullptr) : onRegistry(callback) {}
...
struct xlb_propmate : public xlb_base_prop_ptr {
    Prop_t p2prop;
    xlb_propmate(Prop_t p = nullptr) : p2prop(p) {}

    XLB_SIT get(void* vmate) {
        return reinterpret_cast<xlb_propmate*>(vmate)->p2prop;
    }
};
...
struct xlb_funchandler {
    ...
    xlb_funchandler(func_t _f, xlb_base_func_ptr::ptr_t&& _p) : f(_f) {
...
using xlb_closure_t = std::tuple<const char*, lua_CFunction, xlb_base_func_ptr::ptr_t>;
...
//------------------------------------------------------------------------------------------------
static void xlu_newtypelib( lua_State* L, const char* typeName, xlb_regs_t& typeTable,
                            const xlb_static_member_t& staticMember,
                            int typeSize, int parentTableIndex ) {
...
int xlb_pusharraypointer( lua_State* L, ARR v, xlf_gc gcf, void* place = nullptr ) {
    ...
    //xlb_debug("(%p,eT=%s,%s)\n", pointer, typeid(eT).name(), typeid(ARR).name());
...
template <typename T>
XLB_SIT xlb_touserdata(lua_State* L, int index) {
...
template <typename T>
XLB_SIT xlb_towrap( lua_State* L, int index ) {
    xlb_wrap<T>* wp { xlb_testwrapuservalue( L, index )
                          ? reinterpret_cast<xlb_wrap<T>*>( lua_touserdata( L, index ) )
                          : nullptr };
    return wp;
}
...
template <typename P, typename WP>
XLB_SIT xlb_getwppointer( WP* wp ) {
    assert( wp );
...
//------------------------------------------------------------------------------------------------
template <typename Vt, typename eT, typename WP>
int xlu_arrayitem_setter( lua_State* L ) {  // ud,k,v
...
//------------------------------------------------------------------------------------------------
template <typename Vt, typename eT, typename WP>
int xlu_arrayitem_getter( lua_State* L ) { // ud,k
    constexpr const auto ud_index { 1 };
    constexpr const auto key_index { 2 };
...
void xlu_arraymetaname( std::string& meta_name ) {
    meta_name.clear();
    meta_name = XLB_META_PREFIX_ARRAY;
    meta_name += "(";
    if ( std::is_const_v<eT> ) meta_name += "const ";
...
//------------------------------------------------------------------------------------------------
template <typename Vt, typename eT, typename WP>
int xlu_newarraylib( lua_State* L ) {
...
//------------------------------------------------------------------------------------------------
template <typename T>
void xlu_pointermetaname( std::string& meta_name ) {
    meta_name.clear();
    meta_name = XLB_META_PREFIX_POINTER;
    meta_name += "(";
    if ( std::is_const_v<T> ) meta_name += "const ";
...
template <typename Vt, typename T, typename WP>
int xlu_newpointerlib( lua_State* L) {
    std::string meta_name;
        ...
        if constexpr (! std::is_const_v<e_t> ) {
            ...
            if constexpr ( std::is_array_v<e_t> ) {
                ...
                for ( auto idx = 0; idx < N; ++idx ) {
                    ( isMoved ) ? left[ idx ] = std::move( right[ idx ] )
                        : left[ idx ] = right[ idx ];
                }
...
public:
    ...
    virtual int lvaluereference( lua_State* L ) override {
        int rc { 0 };
        if constexpr ( ! std::is_same_v<deref_at, xlb_notpointer> ) {
            if constexpr ( ! std::is_same_v<xlb_t<deref_at>, void> &&
                           ! std::is_same_v<xlb_t<deref_at>, const void> ) {
    ...
    static void setmeta_array( lua_State* L, int index, lua_Number N = 0 ) {
...
struct xlb_lpointer<Vt, T*, U, Dtor> : public xlb_base_agent // T* or T&
{
    ...
    void getarg( lua_State* L, xlb_ami& ami ) override {
        pointer = nullptr;
        auto got { false };  // maybe we get the pointer but it's value equal nullptr
        //xlb_debug("\n");
...
struct xlb_lpointer<Vt, const T*, U, Dtor> : public xlb_base_agent // const T*, const T&
{
    ...
    void pusharg( lua_State* L, int& rc ) override {
        using deref_at = xlb_at<Vt, deref_t>;
        if constexpr ( std::is_reference_v<U> ) {
            if constexpr ( is_lstruct_v<Vt, deref_at> ) {  // deref_at==xlb_lstruct<const T>
                // xlb_debug("(0x%p)\n", this);
                assert( pointer );
...
//------------------------------------------------------------------------------------------------
template <typename Vt>
struct xlb_lstruct<Vt, void> : public xlb_base_agent {
    using vter = Vt;
    using type = void;
};

template <typename Vt>
struct xlb_lstruct<Vt, const void> : public xlb_base_agent {
    using vter = Vt;
    using type = const void;
};
...
//------------------------------------------------------------------------------------------------
template <typename Vt, typename T>
struct xlb_lstruct : public xlb_base_agent {
    using vter = Vt;
    using type = T;
    using pointer_t = T*;
...
//------------------------------------------------------------------------------------------------
template <typename Vt, typename T>
struct xlb_lnumber : public xlb_base_agent {
    using vter = Vt;
    using type = T;
    T value;
...
//------------------------------------------------------------------------------------------------
template <typename Vt, typename T>
struct xlb_lintegral : public xlb_base_agent {
    using vter = Vt;
    using type = T;
...
// xlb_larray
//------------------------------------------------------------------------------------------------
template <typename Vt, int N, typename T>
struct xlb_larray : public xlb_base_agent {
    using vter = Vt;
    using type = T[ N ];
    using pointer_t = T*;
        ...
        switch ( lua_type( L, ami.arg_index ) ) {
            ...
            case LUA_TUSERDATA: {  // accept array
                ...
                if ( ! wp ) {
                    ...
                    if ( wp ) {
                        //xlb_debug( "(%p,%s,%d)\n", wp, typeid( eT ).name(), xlb_wpisconst( wp ) );
                        if constexpr ( ! std::is_const_v<eT> ) {
                            if ( wp && xlb_wpisconst( wp ) ) {
                                wp = nullptr;
                            }
...
template <typename Vt, typename T>
struct xlb_lboolean : public xlb_base_agent {
    using vter = Vt;
    using type = bool;
    bool value;
...
//------------------------------------------------------------------------------------------------
template <typename Vt>
struct xlb_lfunc : public xlb_base_agent {
    using vter = Vt;
...
struct xlb_dfer : public xpo_base_dfer {
    template <class TUP>
    XLB_SII go( TUP& tup, int dc ) {
        return 0;
    }
};

template <class TUP, class T, int idx>
XLB_SIV xlb_default_assign( TUP& tup, T&& A ) {
    std::get<idx>( tup ) = A;
}
...
struct xlb_setdefaultval<TUP, xlb_idx<idx...>, T...> {
    XLB_SIV go(TUP& tup, int dc, T&&... Dvs) {
        ...
        xlt_each{( 
                (idx>=(dsize-dc)) 
                ? (std::get<tsize - dsize + idx>(tup) = std::forward<T>(Dvs), 
                  //xlb_debug("(%d-%d,%d,%d)\n", tsize, dsize, idx, dc),
                  XLB_STATEMENT_VALUE)
                : XLB_STATEMENT_VALUE  
                )...};
...
XLB_SII xlb_putdefault( TUP& tup, int dc, T&&... Dvs ) {
    constexpr auto tsize = xlb_tupsize_v<TUP>;
...
template <typename TUP>
struct xlb_pushfrv::xlb_tir<void, TUP>
{
    template <typename AIs>
    XLB_SIV go(lua_State* L, TUP& tup, const AIs& ais, int& rc) {}
};

template <int... RPI>
struct xlb_pushfrv::xlb_tir<xlb_derived_rier<RPI...>, std::tuple<>> {
    template <typename AIs>
    XLB_SIV go(lua_State* L, std::tuple<>& tup, const AIs& ais, int& rc) {}
};

template <int... RPI, typename... A>
struct xlb_pushfrv::xlb_tir<xlb_derived_rier<RPI...>, std::tuple<A...>> {
    template <typename AIs>
    XLB_SIV go(lua_State* L, std::tuple<A...>& tup, const AIs& ais, int& rc) {
...
template <typename Vt>
struct xlb_lmemblock : public xlb_base_agent, xlb_memblock {
    using vter = Vt;
...
template <typename T>
using xlb_delete_t = xlb_t<xlb_delete::xlb_tir<T>>;
...
//------------------------------------------------------------------------------------------------
struct xlb_vter {
    template <typename X, typename Partial = xlb_djy>
    struct xlb_tir {};
};  // xlb_vter
...
struct xlb_vter::xlb_tir<T&> {
    using vter = xlb_vter;
    using type = xlt_ifelse_t<std::is_array_v<T>, 
          xlb_t<xlb_vter::xlb_tir<T>>, xlb_lpointer<vter, T*, T&, xlb_delete_t<T>>>;
};
...
struct xlt_cond_floating {
    using type =
        xlt_ifelse_t<std::is_floating_point_v<T>, xlt_fit_floating, xlt_cond_integral_t<T>>;
};
template <typename T>
...
struct xlb_selfobj<void, false_type> {
    template <typename TUP, typename AIs>
    XLB_SIV go(lua_State*, TUP& tup, xlb_ami&, AIs& ais) {}
};
...
//------------------------------------------------------------------------------------------------
struct xlb_invoke {
    template <typename R, bool, typename R_AT, typename Tx, bool, typename FT, typename Idx,
              typename TYPELIST, typename R2L>
    struct xlb_tir {};
};

///< for r pobj->f(...)
template <typename R, typename R_AT, typename Tx, typename FT, typename... A, int... idxs,
          typename R2L>
struct xlb_invoke::xlb_tir<R, false, R_AT, Tx, false, FT, xlb_idx<idxs...>, xlt_typelist<A...>,
                           R2L> {
    template <typename... Arg>
    XLB_SIV go( lua_State* L, xlb_ami& ami, FT f, std::tuple<Arg...>& tup ) {
...
struct xlb_invoke::xlb_tir<void, true, xlt_rvtrait<void>, Tx, false, FT, xlb_idx<idxs...>,
                           xlt_typelist<A...>, R2L> {
    template <typename... Arg>
    XLB_SIV go( lua_State* L, xlb_ami& ami, FT f, std::tuple<Arg...>& tup ) {
...
struct xlb_invoke::xlb_tir<R, false, R_AT, void, true, FT, xlb_idx<idxs...>, xlt_typelist<A...>,
    ...
    template <typename... Arg>
    XLB_SIV go( lua_State* L, xlb_ami& ami, FT f, std::tuple<Arg...>& tup ) {
        auto&& t = std::move( f( ( A ) std::get<idxs>( tup )
                                 ... ) );
...
struct xlb_invoke::xlb_tir<void, true, xlt_rvtrait<void>, void, true, FT, xlb_idx<idxs...>,
                           xlt_typelist<A...>, R2L> {
    template <typename... Arg>
    XLB_SIV go( lua_State* L, xlb_ami&, FT f, std::tuple<Arg...>& tup ) {
...
template <class FT>
struct xlb_fmate : public xlb_base_func_ptr {
    using self_t = xlb_fmate;
    FT fpointer;

    xlb_fmate( FT f = nullptr, onregistry_t callback = nullptr )
        : xlb_base_func_ptr( callback ), fpointer( f ) {}
...
struct xlb_function_ovl : public xlb_regitem {
    ...
    xlb_function_ovl( const char* fn, xlb_ovlfs_t&& fs_ )
        : xlb_regitem( fn ), fs( std::move( fs_ ) ) {}
...
//------------------------------------------------------------------------------------------------
template <typename Lambda>
static constexpr auto xlb_lambda2f( const Lambda& t ) {
...
//------------------------------------------------------------------------------------------------
template <typename Lambda>
struct xlt_lambda2cf_pure {
    static constexpr lua_CFunction cast ( Lambda f ) {
        return xlb_f2cf_pure<xlb_lambda2f( f )>;
    }
};
...
int xlb_propgetter( lua_State* L, void* vmate, xlf_prop vor = xlf_prop::smart ) {  // t,k
    auto rc { 0 };
    using Vt = xpo_get_vter<PO...>;
...
int xlb_propsetter( lua_State* L, void* vmate, xlf_prop vor = xlf_prop::byval ) {  // t,k,v
    ...
    // xlb_debug("%p,%d,%d\n", wp, xlb_wpisconst(wp), std::is_const_v<property_t>);
...
struct xlb_property : public xlb_base_property {
    ...
    xlb_property( const xlb_property& r )
        : getter( std::move( r.getter ) ),
          setter( std::move( r.setter ) ),
          funcer( std::move( r.funcer ) ) {}
    self_t& operator=( xlb_property&& r ) = default;
    virtual ~xlb_property() = default;

    template <typename Prop_t, typename... PO>
    static ptr_t create( Prop_t prop, xpo_readonly ) {
    ...
    template <typename Prop_t, typename... PO>
    static ptr_t create( Prop_t prop, xpo_none ) {
        auto temp = std::make_unique<xlb_property>();
    ...
    template <typename FT>
    static ptr_t create( lua_CFunction cf, FT f ) {
        auto temp = std::make_unique<xlb_property>();
...
//------------------------------------------------------------------------------------------------
#define XLB_BF3ARG( T, b ) \
#b, []( T * obj ) { return obj->b; }, []( T * obj, int64_t v ) { obj->b = v; }
...
struct xlb_bitfieldprop : public xlb_base_property {
    ...
    static ptr_t create( Reader_t r, Writer_t w ) {
...
struct xlb_const : public xlb_regitem {
    ...
    template <class T>
    xlb_const( const char* name, T v,
               std::enable_if_t<std::is_integral_v<T> || std::is_enum_v<T>>* = 0 )
    ...
    template <class T>
    xlb_const( const char* name, const T& v, unsigned len = sizeof( T ),
    ...
    virtual void registry( xlb_ns* parentNS ) override {
        ...
        switch ( vtype ) {
            ...
            case 1:
                ...
                break;
            case 2:
                lua_pushlstring( L, reinterpret_cast<const char*>( wcs ),
                                 wcslen( wcs ) * sizeof( wchar_t ) );
                break;
            case 3:
...
//------------------------------------------------------------------------------------------------
template <typename T, typename Vt = xlb_vter>
struct xlb_cref : public xlb_regitem {
    const T& cv;
...
struct xlb_base_class : public xlb_regitem {
        ...
        if ( iter != memberMap.end() ) {
            ...
            switch ( prop.type ) {
                ...
                case xlb_base_property::bitfield:
                    ...
                    nfound = 1;
                    break;
                case xlb_base_property::readonly: break;  // do nothing for readonly
            }
        }  // if found
    ...
    static X* return_ptt() { // pointer to type
        return { (X*) nullptr };
    }
    ...
    template <typename FT, class... PO>
    rr_t method( const char* func_name, FT f, PO... ) {
        memberMap[ func_name ] = xlb_property::create( xlb_f2cf_static<FT, PO...>, f );
        return std::move( *this );
    }

    template <auto f, class... PO>
    rr_t method( const char* func_name, PO... ) {
        using FT = decltype( f );
        memberMap[ func_name ] = xlb_property::create( xlb_f2cf_static<FT, PO...>, f );
        return std::move( *this );
    }
    ...
    template <class Prop_t, class... PO>
    rr_t property( const char prop_name[], Prop_t prop, PO... ) {
...
static int xlb_pvtostring( lua_State* L ) {
    auto rc { 1 };
    xlb_lpointer<xlb_vter, void*, void*, std::default_delete<void*>> lpointer;
    xlb_ami ami( lua_gettop( L ) );
    ami.arg_index = 1;
